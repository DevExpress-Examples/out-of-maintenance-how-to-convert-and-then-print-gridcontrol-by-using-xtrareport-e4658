    ...
    public class ReportGeneratonHelper {
        ...
        private List<ColumnInfo> GetColumnsInfo(GridControl grid, int pagewidth) {
            ...
            foreach(GridColumn dataColumn in visibleColumns) {
                ColumnInfo column = new ColumnInfo(dataColumn) { ColumnCaption = string.IsNullOrEmpty(dataColumn.HeaderCaption.ToString()) ? dataColumn.FieldName : dataColumn.HeaderCaption.ToString(), ColumnWidth = ((int)pagewidth / visibleColumns.Count), FieldName = dataColumn.FieldName, IsVisible = true };
                columns.Add(column);
            }
            return columns;
